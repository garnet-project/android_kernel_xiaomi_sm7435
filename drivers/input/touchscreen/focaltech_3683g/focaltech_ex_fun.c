...
static ssize_t TP_charger_store(struct file *filp, const char __user *ubuf,
				size_t count, loff_t *ppos)
{
	char buf[20];
	u32 tmp;
...
static ssize_t fts_pocket_mode_store(struct file *filp, const char __user *ubuf,
				     size_t count, loff_t *ppos)
{
	char buf[20];
	u32 tmp;
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		FTS_ERROR("Pre-pocket_mode = %d, Enter pocket mode",
			  ts_data->pocket_mode);
		...
		FTS_ERROR("Pre-pocket_mode = %d, Exit pocket mode",
			  ts_data->pocket_mode);
...
static ssize_t fts_edge_mode_store(struct file *filp, const char __user *ubuf,
				   size_t count, loff_t *ppos)
{
	char buf[20];
	int ret = 0;
...
static ssize_t fts_game_mode_store(struct file *filp, const char __user *ubuf,
				   size_t count, loff_t *ppos)
{
	char buf[20];
	int ret = 0;
...
static ssize_t fts_gesture_store(struct file *filp, const char __user *ubuf,
				 size_t count, loff_t *ppos)
{
	char buf[20];
	u32 tmp;
...
static ssize_t fts_diff_mode_store(struct file *filp, const char __user *ubuf,
				   size_t count, loff_t *ppos)
{
	char buf[20];
	u32 tmp;
	...
	if (!!tmp ^ ts_data->fwdbg_support) {
		...
		FTS_INFO("value(%d)==fwdbg_support(%d)", !!tmp,
			 ts_data->fwdbg_support);
	mutex_unlock(&ts_data->input_dev->mutex);
...
static int fts_fw_info_show(struct seq_file *s, void *unused)
{
	...
	FTS_INFO("probe Test %lldms taken",
		 ktime_to_ms(ktime_sub(ktime_get(), ts_data->probe_time)));
	FTS_INFO("resume Test %lldms taken",
		 ktime_to_ms(ktime_sub(ktime_get(), ts_data->resume_time)));
	mutex_lock(&ts_data->input_dev->mutex);
	ret = fts_read_reg(FTS_REG_FW_VER, &fwver);
	if (ts_data->lcd_status) {
		if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00)) {
			seq_printf(s,
				   "get tp fw version fail!fwver=%02x,ret=%d\n",
				   fwver, ret);
...
static int fts_driver_info_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "reset gpio:%d,int gpio:%d,irq:%d\n", pdata->reset_gpio,
	...
	if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00)) {
		num_read_chars =
			snprintf(buf, PAGE_SIZE,
				 "get tp fw version fail!ret=%d,fwver=%02x\n",
				 ret, fwver);
		ret = fts_read_reg(0xA3, &fwver);
		FTS_INFO("A3 = %x\n", fwver);
		fts_read(&cmd, 1, data, 2);
...
int fts_procfs_init(void)
{
	...
	if (!proc_touchpanel) {
		...
		proc_create_data("gesture_code", 0664, proc_touchpanel,
				 &fts_gesture_point_fops, NULL);
		proc_create_data("game_mode", 0664, proc_touchpanel,
				 &fts_game_mode_fops, NULL);
		proc_create_data("edge_mode", 0664, proc_touchpanel,
				 &fts_edge_mode_fops, NULL);
		proc_create_data("pocket_mode", 0664, proc_touchpanel,
				 &fts_pocket_mode_fops, NULL);
		proc_create_data("TP_charger_flags", 0664, proc_touchpanel,
				 &TP_charger_fops, NULL);
		proc_create_data("diff_mode", 0664, proc_touchpanel,
				 &fts_diff_mode_fops, NULL);
		proc_create_data("fw_version", 0664, proc_touchpanel,
				 &fts_fw_info_fops, NULL);
		proc_create_data("driver_info", 0664, proc_touchpanel,
				 &fts_driver_info_fops, NULL);
