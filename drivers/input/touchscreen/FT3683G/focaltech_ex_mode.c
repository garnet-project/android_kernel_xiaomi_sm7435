...
static int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
	...
	case MODE_COVER:
		...
		break;
	case MODE_GAME:
		ret = fts_write_reg(FTS_REG_GAME_MODE_EN, m_val);
		if (ret < 0) {
			FTS_ERROR("MODE_GAME switch to %d fail", m_val);
		}
		break;
	case MODE_EDGE:
		...
		break;
	case MODE_POCKET:
		ret = fts_write_reg(FTS_REG_POWER_MODE, m_val);
		if (ret < 0) {
			FTS_ERROR("MODE_POCKET switch to %d fail", m_val);
		}
		break;
	default:
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf + count, PAGE_SIZE, "Game Reg(0x8A):%d\n", val);
				   ...
				   const char *buf, size_t count)
{
	...
	if (FTS_SYSFS_ECHO_ON(buf)) {
		ret = fts_ex_mode_switch(MODE_GAME, 5);
		if (ret >= 0) {
			FTS_DEBUG("Enter game mode");
			ts_data->game_mode = ENABLE;
		}
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		ret = fts_ex_mode_switch(MODE_GAME, 0);
		if (ret >= 0) {
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf + count, PAGE_SIZE, "Edge Mode:\n");
				   ...
				   const char *buf, size_t count)
{
	...
	int value = 0;
	sscanf(buf, "%d", &value);
	ret = fts_ex_mode_switch(MODE_EDGE, value);
	if (ret >= 0) {
		FTS_DEBUG("Enter Edge mode, value = %d", value);
	}
	return count;
}
static ssize_t fts_pocket_mode_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	int count = 0;
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
	...
	mutex_lock(&input_dev->mutex);
	count = snprintf(buf + count, PAGE_SIZE, "Pocket Mode:%s\n",
...
static ssize_t fts_pocket_mode_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	struct fts_ts_data *ts_data = dev_get_drvdata(dev);
	...
	mutex_lock(&input_dev->mutex);
	if (FTS_SYSFS_ECHO_ON(buf)) {
		FTS_DEBUG("Pre-pocket_mode = %d, Enter pocket mode",
			  ts_data->pocket_mode);
		fts_ex_mode_switch(MODE_POCKET, 2);
		ts_data->pocket_mode = ENABLE;
		msleep(10); //Ensure that the firmware completes the pocket mode process.
	} else if (FTS_SYSFS_ECHO_OFF(buf)) {
		FTS_DEBUG("Pre-pocket_mode = %d, Exit pocket mode",
			  ts_data->pocket_mode);
		fts_ex_mode_switch(MODE_POCKET, 0);
		msleep(10);
...
static DEVICE_ATTR(fts_game_mode, S_IRUGO | S_IWUSR, fts_game_mode_show,
		   fts_game_mode_store);
static DEVICE_ATTR(fts_edge_mode, S_IRUGO | S_IWUSR, fts_edge_mode_show,
		   fts_edge_mode_store);
static DEVICE_ATTR(fts_pocket_mode, S_IRUGO | S_IWUSR, fts_pocket_mode_show,
		   fts_pocket_mode_store);
static struct attribute *fts_touch_mode_attrs[] = {
	&dev_attr_fts_glove_mode.attr,	&dev_attr_fts_cover_mode.attr,
	&dev_attr_fts_game_mode.attr,	&dev_attr_fts_edge_mode.attr,
	&dev_attr_fts_pocket_mode.attr, NULL,
};
